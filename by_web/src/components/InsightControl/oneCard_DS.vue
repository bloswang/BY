<template>
  <div class="w">
    <el-breadcrumb separator=">" class="navigation">
      ...
      <el-breadcrumb-item>一卡通上机消费分析</el-breadcrumb-item>
    </el-breadcrumb>

    <el-row type="flex" justify="space-around">
      <el-col :span="24" class="ec_bg">
        <h5 class="ec_h5 all_num">
          {{all_select_year}}年 上机总消费:{{all_Money}}元
        ...
        <h5 class="ec_h5" >
          ...
          <div class="ec_h5_data" style="float: right; margin-left: 20px">
            <el-date-picker
              ...
              size='mini'
              format="yyyy-MM-dd"
              @change="SJ_count_time_S_change"
              v-model="SJ_count_time_S"
              type="date"
              placeholder="选择年">
            </el-date-picker>
            至
            <el-date-picker
              ...
              size='mini'
              format="yyyy-MM-dd"
              @change="SJ_count_time_N_change"
              v-model="SJ_count_time_N"
              type="date"
              placeholder="选择月">
            </el-date-picker>
          </div>
        </h5>
        <div class="ec_height" id="oneCard_SJ_count">
        </div>
      </el-col>
    </el-row>
        ...
        <h5 class="ec_h5" >
          ...
          <div class="ec_h5_data" style="float: right; margin-left: 20px">
            <el-date-picker
              ...
              size='mini'
              format="yyyy-MM-dd"
              @change="SJ_person_time_S_change"
              v-model="SJ_person_time_S"
              type="date"
              placeholder="选择年">
            </el-date-picker>
            至
            <el-date-picker
              ...
              size='mini'
              format="yyyy-MM-dd"
              @change="SJ_person_time_N_change"
              v-model="SJ_person_time_N"
              type="date"
              placeholder="选择月">
            </el-date-picker>
          </div>
        </h5>
        <div class="ec_height" id="oneCard_SJ_person">
        </div>
      </el-col>
    </el-row>
  </div>
  ...
  export default {
    data(){
      return{
        ...
        //各类人员在一个时间段内上机的金额
        oneCard_SJ_count:'',
        SJ_count_time_S:'2017-01-01',
        SJ_count_time_N:'2018-01-01',
        SJ_count:[],
        SJ_name:[],

        //各类人员在一个时间段内上机的人数
        oneCard_SJ_person:'',
        SJ_person_time_S:'2017-01-01',
        SJ_person_time_N:'2018-01-01',
        SJ_person:[],
        SJ_name1:[],
      }
    },
    methods:{
      make_time(){
        ...
        vm.SJ_count_time_S = time.year + '-' +'01'+'-'+'01'
        vm.SJ_count_time_N = time.year + '-' +time.month+'-'+time.day
        vm.SJ_person_time_S = time.year + '-' +'01'+'-'+'01'
        vm.SJ_person_time_N = time.year + '-' +time.month+'-'+time.day
      },
      //总消费时间选择
      all_change_year(val){
        var vm = this
      ...
      oneCard_SJ_count_fn(){
        ...
        this.oneCard_SJ_count.setOption(option);
      },
      SJ_count_time_S_change(val){
        if(val == undefined){
          return
        }else if(val >this.SJ_count_time_N){
          if(this.SJ_count_time_N!==''){
            this.$message.error('开始时间不能大于结束时间哦！');
            this.SJ_count_time_S = ''
            return
          }else{
            this.SJ_count_time_S = val
          }
        }else{
          this.SJ_count_time_S = val
          this.count_init_fn(this.SJ_count_time_S,this.SJ_count_time_N)
        }

      },
      SJ_count_time_N_change(val){
        if(val == undefined){
          return
        }else if(val<this.SJ_count_time_S){
          this.$message.error('结束时间不能小于开始时间哦！');
          this.SJ_count_time_N = ''
          return
        }else{
          this.SJ_count_time_N = val
          this.count_init_fn(this.SJ_count_time_S,this.SJ_count_time_N)
        }
      ...
      count_init_fn(start,end){
        let vm = this
        vm.$axios.request("post","oneCard/onlineConsume",{startTime:start,endTime:end},function(data){
          vm.SJ_name = []
          vm.SJ_count = []
          for(var i=0;i<data.result.length;i++){
            vm.SJ_name.push(data.result[i].cardtype)
            vm.SJ_count.push(data.result[i].count)
          }
          vm.oneCard_SJ_count_fn()
        },function(err){
          console.log(err);
        })
      },
      ...
      oneCard_SJ_person_fn(){
        ...
        this.oneCard_SJ_person.setOption(option);
      },
      SJ_person_time_S_change(val){
        if(val == undefined){
          return
        }else if(val >this.SJ_person_time_N){
          if(this.SJ_person_time_N!==''){
            this.$message.error('开始时间不能大于结束时间哦！');
            this.SJ_person_time_S = ''
            return
          }else{
            this.SJ_person_time_S = val
          }
        }else{
          this.SJ_person_time_S = val
          this.person_init_fn(this.SJ_person_time_S,this.SJ_person_time_N)
        }

      },
      SJ_person_time_N_change(val){
        if(val == undefined){
          return
        }else if(val<this.SJ_person_time_S){
          this.$message.error('结束时间不能小于开始时间哦！');
          this.SJ_person_time_N = ''
          return
        }else{
          this.SJ_person_time_N = val
          this.person_init_fn(this.SJ_person_time_S,this.SJ_person_time_N)
        }

      },
      person_init_fn(start,end){
        let vm = this
        vm.$axios.request("post","oneCard/onlineCount",{startTime:start,endTime:end},function(data){
          vm.SJ_name1 = []
          vm.SJ_person = []
          for(var i=0;i<data.result.length;i++){
            vm.SJ_name1.push(data.result[i].cardtype)
            vm.SJ_person.push(data.result[i].count)
          }
          vm.oneCard_SJ_person_fn()
        },function(err){
          console.log(err);
        })
      },
    ...
    mounted(){
      ...
      this.oneCard_SJ_count = this.$echarts.init(document.getElementById('oneCard_SJ_count'), 'customed');
      this.oneCard_SJ_person = this.$echarts.init(document.getElementById('oneCard_SJ_person'), 'customed');
    },
    created(){
      window.scrollTo(0,0);
      var vm = this
